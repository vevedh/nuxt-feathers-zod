...
const windowsInstallFingerprint = read('scripts/lib/windows-install-fingerprint.mjs')
...
requireText(windowsInstaller, 'schemaVersion: 3', 'dependency-scoped Windows install state schema')
requireText(windowsInstaller, 'shouldAttemptInPlaceReconciliation({', 'generalized non-destructive Windows dependency reconciliation')
requireText(windowsInstaller, 'Refusing destructive node_modules cleanup', 'frozen-lock mismatch preservation')
requireText(windowsInstallFingerprint, 'dependencyManifestSha256', 'dependency-only Windows install fingerprint')
requireText(windowsInstallFingerprint, 'lockfileSha256', 'Windows install lockfile fingerprint')
