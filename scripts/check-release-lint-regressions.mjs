...
requireText(databaseRegistry, 'a-z0-9+.-]*:\\/\\/[^\\s', 'provider-neutral database URL redaction regex')
requireText(databaseRegistry, "'[redacted-database-url]'", 'database URL redaction replacement')
requireText(databaseRegistry, 'const connectionConfig = handle.config', 'connector config discriminant binding')
requireText(databaseRegistry, "if (connectionConfig.provider === 'mongodb') {\n        result = await", 'provider connector branch indentation')
forbidText(databaseRegistry, "if (handle.provider === 'mongodb') {\n        result = await", 'detached handle-provider connector dispatch')
forbidText(databaseRegistry, "if (handle.type === 'mongodb') {\n        result = await", 'engine-specific connector dispatch')
