...
This page replaces the former navigation-only placeholder with a practical developer reference for the frontend composables (`useService`, `useFeathers`, `useProtectedService`, `useAuth`). It explains the option, shows how to configure it in `nuxt.config.ts`, and gives a minimal usage example.
