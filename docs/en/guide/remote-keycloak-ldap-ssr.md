...
This guide describes the recommended SSR model for a Nuxt 4 application using Keycloak SSO in the browser, a remote Feathers backend consumed through NFZ `6.8.0`, and a backend `keycloak-ldap` strategy to resolve the LDAP/Active Directory user.

The model keeps three responsibilities strictly separated:

```txt
Keycloak = Nuxt client only
NFZ 6.8.0 = direct remote Feathers client
...
This example uses Nuxt 4 SSR, Quasar 2, UnoCSS, Pinia, direct NFZ `6.8.0` remote mode, client-only Keycloak and automatic LDAP synchronization.
