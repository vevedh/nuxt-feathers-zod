...
La console ajoute les services Feathers internes utilisés par le Builder et les diagnostics.
...
|---|---:|---|
| `enabled` | `false` | Enregistre les services Feathers `nfz/*` |
| `basePath` | `/console` | Chemin des pages de console lorsque celles-ci sont montées |
| `allowWrite` | `true` en développement, sinon `false` | Autorise les modifications de schémas, manifestes, RBAC et presets |
| `servicesDirs` | option racine | Remplace les dossiers inspectés par la console |
| `legacyNitroRoutes` | `true` dans la branche 6.x | Conserve les façades dépréciées `/api/nfz/**` |

Les nouveaux projets devraient définir `legacyNitroRoutes: false`. Le client canonique utilise `useBuilderClient()` ou `client.service('nfz/...')`.
