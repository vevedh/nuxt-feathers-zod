...
Ce guide est le modèle simple recommandé pour NFZ `6.8.0` lorsqu'une application Nuxt 4 doit utiliser Keycloak SSO et un utilisateur LDAP/Active Directory enrichi.
...
Cet exemple reprend le modèle validé : Nuxt 4 SPA, Quasar, UnoCSS, Pinia, Keycloak client-only, NFZ `6.8.0` remote direct et synchronisation LDAP automatique après `keycloak.init()`.
