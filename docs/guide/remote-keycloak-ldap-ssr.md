...
Ce guide décrit le modèle SSR recommandé pour une application Nuxt 4 qui utilise Keycloak SSO côté navigateur, un backend Feathers distant exposé via NFZ `6.7.51`, et une stratégie backend `keycloak-ldap` pour résoudre l'utilisateur LDAP/Active Directory.
...
Cet exemple reprend la structure Nuxt 4 SSR, Quasar 2, UnoCSS, Pinia, NFZ `6.7.51` remote direct, Keycloak client-only et synchronisation LDAP automatique.
