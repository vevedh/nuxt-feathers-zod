...
function regexPatchNuxtConfig(raw: string, servicesDirName: string, mongoDefaultUrl?: string): PatchResult {
  ...
  const hasFeathersConfig = /\bfeathers\s*:/.test(raw)
  const hasDatabaseConfig = /\bdatabase\s*:/.test(raw)
  ...
  const hasFeathersBlock = /\bfeathers\s*:/.test(next)
  const hasLegacyServicesDir = /\bservicesDir\s*:/.test(next)
  const hasServicesDirs = /\bservicesDirs\s*:/.test(next)
