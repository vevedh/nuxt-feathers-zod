...
async function astPatchNuxtConfig(raw: string, servicesDirName: string, mongoDefaultUrl?: string): Promise<PatchResult | null> {
...
export async function initProject(opts: InitProjectOptions) {
  ...
  const { changed, content, reason } = await patchNuxtConfig(raw, servicesDirName, mongoDefaultUrl, { printDiff: opts.printDiff, label: configPath })
...
${content}`)
  ...
  if (database === 'mongodb') {
    ...
    const composePath = opts.composePath ? resolve(opts.projectRoot, opts.composePath) : join(opts.projectRoot, 'docker-compose.yaml')

    // Matches the reference .env provided by the user
    const envTemplate = `MONGODB_URL=${mongoDefaultUrl}\nMONGODB_ADMIN=${mongoUser}\nMONGODB_PASSWORD=${mongoPassword}\nFIRST_USER=nfzuser\nFIRST_PASSWORD=nfzpass\n`
