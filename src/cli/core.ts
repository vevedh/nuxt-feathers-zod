...
function patchFeathersObjectLiteral(feathersObj: string, patch: NuxtConfigPatch): string {
  ...
  if (patch.clientMode === 'embedded' || patch.embedded) {
    ...
    setObjectProp(entries, 'client', `{ mode: 'embedded' }`)
  }
  ...
  if (isRemote) {
    ...
    removeObjectProp(entries, 'transports')
...
export async function generateFileService(opts: GenerateFileServiceOptions) {
  const serviceNameKebab = normalizeServiceName(opts.name)
  const ids = createServiceIds(serviceNameKebab)
  const servicePath = normalizeServicePath(opts.servicePath ?? serviceNameKebab)
  const storageDir = String(opts.storageDir || `storage/${serviceNameKebab}`).trim().replace(/\\/g, '/')
  const dir = join(opts.servicesDir, serviceNameKebab)
  const schemaFile = join(dir, `${serviceNameKebab}.schema.ts`)
  const classFile = join(dir, `${serviceNameKebab}.class.ts`)
  const sharedFile = join(dir, `${serviceNameKebab}.shared.ts`)
  ...
  const files: Array<{ path: string, content: string }> = [
    { path: schemaFile, content: renderFileServiceSchema(ids) },
    { path: classFile, content: renderFileServiceClass(ids, storageDir) },
    { path: sharedFile, content: renderFileServiceShared(ids, servicePath) },
    { path: serviceFile, content: renderFileService(ids, servicePath, opts.auth, opts.docs) },
  ]
  ...
  await writeServiceManifest(opts.servicesDir, {
    ...
    auth: opts.auth,
    custom: true,
    methods: ['find', 'get', 'remove', 'upload', 'download'],
  ...
  if (!opts.dry)
    consola.success(`Generated file service '${serviceNameKebab}' in ${relativeToCwd(dir)}`)
}
...
export const ${base}DownloadResultSchema = ${base}FileSchema.extend({
  dataBase64: z.string().min(1),
})
export type ${Base}DownloadResult = z.infer<typeof ${base}DownloadResultSchema>
`
}

function renderFileServiceClass(ids: ReturnType<typeof createServiceIds>, storageDir: string) {
  const Base = ids.basePascal
  const serviceClass = `${Base}Service`
  const serviceName = ids.serviceNameKebab
  const base = ids.baseCamel
...
export class ${serviceClass} {
  ...
  private async ensureStorageDir() {
    const root = this.resolveStorageDir()
  ...
  async upload(data: ${Base}UploadData, _params?: Params): Promise<${Base}File> {
    ...
    await writeFile(this.dataPath(root, id), buffer)
    await writeFile(this.metaPath(root, id), JSON.stringify(descriptor, null, 2), 'utf8')
    return descriptor
  }
...
function renderFileServiceShared(ids: ReturnType<typeof createServiceIds>, servicePath: string) {
  const Base = ids.basePascal
  const base = ids.baseCamel
  const serviceName = ids.serviceNameKebab
  return `// ! Generated by nuxt-feathers-zod - local file upload/download service template
...
export const ${base}Methods = ['find', 'get', 'remove', 'upload', 'download'] as const

export type ${Base}ClientService = RestService & {
  upload(data: ${Base}UploadData, params?: Params): Promise<${Base}File>
  download(data: ${Base}DownloadData, params?: Params): Promise<${Base}DownloadResult>
}

function attach_upload(remote: any) {
  if (typeof remote?.upload === 'function') return
  if (typeof remote?.request === 'function') {
    remote.upload = (data: ${Base}UploadData, params?: Params) => remote.request({ method: 'upload', body: data }, params)
    return
  }
  remote.upload = async (data: ${Base}UploadData, params?: Params): Promise<${Base}File> => {
    const cfg: any = useRuntimeConfig()
    const pub = cfg.public?._feathers ?? {}
    const baseURL = pub.restUrl ?? pub.baseURL ?? pub.url ?? ''
    const prefix = pub.rest?.path ?? pub.restPath ?? pub.prefix ?? '/feathers'
    const url = joinURL(baseURL, prefix, '${servicePath}', 'upload')
    const headers: Record<string, string> = {}
    const auth = (params as any)?.headers?.authorization || (params as any)?.headers?.Authorization
    if (auth) headers.authorization = auth
    return await $fetch<${Base}File>(url, { method: 'POST', body: data, headers })
  }
}

function attach_download(remote: any) {
  if (typeof remote?.download === 'function') return
  if (typeof remote?.request === 'function') {
    remote.download = (data: ${Base}DownloadData, params?: Params) => remote.request({ method: 'download', body: data }, params)
    return
  }
  remote.download = async (data: ${Base}DownloadData, params?: Params): Promise<${Base}DownloadResult> => {
    const cfg: any = useRuntimeConfig()
    const pub = cfg.public?._feathers ?? {}
    const baseURL = pub.restUrl ?? pub.baseURL ?? pub.url ?? ''
    const prefix = pub.rest?.path ?? pub.restPath ?? pub.prefix ?? '/feathers'
    const url = joinURL(baseURL, prefix, '${servicePath}', 'download')
    const headers: Record<string, string> = {}
    const auth = (params as any)?.headers?.authorization || (params as any)?.headers?.Authorization
    if (auth) headers.authorization = auth
    return await $fetch<${Base}DownloadResult>(url, { method: 'POST', body: data, headers })
  }
}
...
export function ${base}Client(client: ClientApplication) {
  ...
  if (import.meta.server) {
    client.use(${base}Path, remote, { methods: ['find', 'get', 'remove'] })
    return
  }
  attach_upload(remote)
  attach_download(remote)
  client.use(${base}Path, remote, { methods: ['find', 'get', 'remove', 'upload', 'download'] })
}
...
function renderFileService(ids: ReturnType<typeof createServiceIds>, servicePath: string, auth: boolean, docs: boolean) {
  const Base = ids.basePascal
  const base = ids.baseCamel
  const serviceName = ids.serviceNameKebab
  const serviceClass = `${Base}Service`
...
import { ${serviceClass} } from './${serviceName}.class'
import { ${base}DownloadDataSchema, ${base}DownloadResultSchema, ${base}FileSchema, ${base}UploadDataSchema } from './${serviceName}.schema'
...
export function ${base}(app: Application) {
  app.use(${base}Path, new ${serviceClass}(app), {
    methods: ${base}Methods as unknown as string[],
    events: [],${docsBlock}
  })
  ...
  app.service(${base}Path).hooks({
    around: {
      all: [],
      upload: [async (context) => { ${base}FileSchema.parse(context.result); return context }],
      download: [async (context) => { ${base}DownloadResultSchema.parse(context.result); return context }],
    },
    before: {
${hookBefore}      upload: [schemaHooks.validateData(${base}UploadDataSchema)],
      download: [schemaHooks.validateData(${base}DownloadDataSchema)],
    },
    after: {
      all: [],
    },
