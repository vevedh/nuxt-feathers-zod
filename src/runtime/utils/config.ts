...
export function getPublicBuilderServices(publicConfig?: PublicFeathersRuntimeLike | null) {
  const configured = getPublicBuilderConfig(publicConfig)?.services ?? {}
  return {
    discovery: String(configured.discovery || 'nfz/services'),
    schemas: String(configured.schemas || 'nfz/schemas'),
    manifest: String(configured.manifest || 'nfz/manifest'),
    builder: String(configured.builder || 'nfz/builder'),
    status: String(configured.status || 'nfz/status'),
...
/** @deprecated Use getPublicBuilderServices() and the Feathers client. */
export function getPublicBuilderBasePath(publicConfig?: PublicFeathersRuntimeLike | null): string {
  ...
  return normalizeLeadingPath(String(builder?.legacyNitro?.basePath || '/api/nfz').trim(), '/api/nfz')
}

/** @deprecated Legacy Nitro compatibility metadata. */
...
/** @deprecated Legacy Nitro compatibility path helper. */
...
/** @deprecated Use getPublicBuilderServices() and client.service(...). */
