...
/**
 * Generates the TypeScript declaration file for `feathers/server/server.ts`.
 *
 * Keep ALL types in this .d.ts so the runtime file can remain plain-JS parseable.
 */
export function getServerDtsContents(options: ResolvedOptions) {
  return (): string => {
...
${put(auth, `import type { ${entityImport?.as} } from '${entityImport?.from}'`)}
...
${put(auth, `declare module '@feathersjs/feathers' {
  interface Params {
