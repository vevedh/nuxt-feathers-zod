...
import { discoverServerServices } from '../../discovery'
...
function buildRouterBlock({ koa, exp, sio, restPath }: { koa: boolean, exp: boolean, sio: boolean, restPath: string | undefined }): string {
  ...
  if (sio)
    lines.push('  await createSocketIoRouter(app)')

  if (!lines.length)
    lines.push('  return')

  return `const nfzCreateRouters = async (app) => {\n${lines.join('\n')}\n}\n`
}

export function getServerPluginContents(options: ResolvedOptions) {
  return async (): Promise<string> => {
    const services = setImportsMeta(await discoverServerServices(options.servicesDirs))
...
`
    ...
    const keycloakInitBlock = keycloakEnabled
      ? `const nfzInitKeycloak = async (app, config) => {
  const keycloak = config?.keycloak
  if (!keycloak?.serverUrl || !keycloak?.realm || !keycloak?.clientId)
    throw new Error('[nuxt-feathers-zod] Missing private Keycloak runtime configuration.')
  await setupKeycloakBridge(app, keycloak)
}
`
      : 'const nfzInitKeycloak = undefined\n'

    const baseConfigLiteral = JSON.stringify({
      transports: {
      ...
      console: options.console ? {
        enabled: true,
        allowWrite: options.console.allowWrite,
        servicesDirs: options.console.servicesDirs,
      } : false,
    }, null, 2)

    return `// ! Generated by nuxt-feathers-zod - do not change manually
${put(keycloakEnabled, `import { setupKeycloakBridge } from './keycloak'`)}
${put(Boolean(options.console?.enabled), `import { registerNfzConsoleServices } from 'nuxt-feathers-zod/server-console-services'`)}
...
${put(exp, `import { expressErrorHandler } from '@vevedh/feathers-nitro/handlers'`)}
${put(Boolean(routers), `import { ${routers} } from '@vevedh/feathers-nitro/routers'`)}
import { defineNitroPlugin, useRuntimeConfig } from 'nitropack/runtime'
${modules.map(module => module.meta.import).join('\n')}

const nfzBaseServerConfig = ${baseConfigLiteral}
...
export default defineNitroPlugin(async (nitroApp) => {
  const privateConfig = useRuntimeConfig()?._feathers || {}
  const nfzServerConfig = {
    ...nfzBaseServerConfig,
    ...privateConfig,
