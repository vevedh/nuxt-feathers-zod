...
describe('resolvePublicRuntimeConfig', () => {
  ...
  it('exposes public builder metadata and helper paths', async () => {
    ...
    const pub = resolvePublicRuntimeConfig(await resolveOptions({
      ...
      templates: undefined,
      devtools: true,
      console: { enabled: true, allowWrite: false },
    } as any, nuxtMock))

    expect(pub.builder?.enabled).toBe(true)
    expect(pub.builder?.transport).toBe('feathers')
    expect(getPublicBuilderServices({ _feathers: pub } as any)).toEqual(expect.objectContaining({
      discovery: 'nfz/services',
      schemas: 'nfz/schemas',
      builder: 'nfz/builder',
    }))
    expect(getPublicBuilderBasePath({ _feathers: pub } as any)).toBe('/api/nfz')
  ...
  it('can disable all legacy Nitro Builder metadata while keeping Feathers services enabled', async () => {
    const pub = resolvePublicRuntimeConfig(await resolveOptions({
      transports: { rest: { framework: 'express', path: '/feathers' }, websocket: false },
      database: {},
      servicesDirs: [],
      ...
      templates: undefined,
      devtools: true,
      console: { enabled: true, allowWrite: false, legacyNitroRoutes: false },
    } as any, nuxtMock))

    expect(pub.builder?.enabled).toBe(true)
    expect(pub.builder?.services?.schemas).toBe('nfz/schemas')
    expect(pub.builder?.legacyNitro?.enabled).toBe(false)
    expect(pub.builder?.routes).toEqual([])
  })


  it('does not advertise Builder routes when the console is disabled', async () => {
    const pub = resolvePublicRuntimeConfig(await resolveOptions({
      transports: { rest: { framework: 'express', path: '/feathers' }, websocket: false },
      database: {},
      servicesDirs: [],
    ...
    expect(pub.builder?.enabled).toBe(false)
    expect(pub.builder?.routes).toEqual([])
  })
