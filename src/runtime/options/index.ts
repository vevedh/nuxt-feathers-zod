...
export function resolvePublicRuntimeConfig(options: ResolvedOptions): FeathersPublicRuntimeConfig {
  ...
  return {
    ...
    database: options.database?.mongo
      ...
      : undefined,
    builder: {
      enabled: Boolean(options.console?.enabled),
      transport: 'feathers',
      basePath: 'nfz',
      auth: {
        enabled: Boolean(options.auth || options.keycloak),
        authenticate: Boolean(options.auth || options.keycloak),
      },
      services: {
      ...
      routes: options.console?.enabled && options.console.legacyNitroRoutes !== false
        ? [
