...
export function resolveSchemaFile(
  ...
  for (const d of servicesDirs) {
    const servicesRoot = resolveServicesDir(projectRoot, d)
    candidates.push(join(servicesRoot, kebab, `${kebab}.schema.ts`))
  }
  ...
  const fallbackServicesDir = resolveServicesDir(projectRoot, servicesDirs[0] ?? 'services')
...
export function readManifest(projectRoot: string, servicesDirs: string[]) {
  ...
  const baseDir = resolveServicesDir(projectRoot, servicesDirs[0] ?? 'services')
  const manifestPath = join(baseDir, '.nfz', 'manifest.json')
...
export function writeManifestFields(
  ...
  const baseDir = resolveServicesDir(projectRoot, servicesDirs[0] ?? 'services')
  const manifestDir = join(baseDir, '.nfz')
