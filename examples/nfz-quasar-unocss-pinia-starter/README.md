...
Starter **Nuxt 4 + Quasar 2 + UnoCSS + Pinia + nuxt-feathers-zod 6.7.37** avec MongoDB local prêt à l’emploi.
...
Si la version 6.7.37 n’est pas encore publiée sur npm, utilise ton tarball local :

```bash
# depuis le dépôt nuxt-feathers-zod 6.7.37
bun install
bun run build
npm pack
...
bun add ../nuxt-feathers-zod/nuxt-feathers-zod-6.7.37.tgz
```
...
Depuis `6.7.37`, cette couche injecte explicitement le JWT dans les appels Feathers protégés :
...
## Patch 6.7.37 - Hydration / Quasar ripple cleanup
