...
interface MessageSeedRow {
  text?: string
}

interface FindRows<T> {
  data?: T[]
}

interface SeedUserService {
  find(
    params: { query: { userId: string, $limit: number }, paginate: false },
  ): Promise<UserSeedRow[] | FindRows<UserSeedRow>>
  create(data: { userId: string, password: string, roles: string[] }): Promise<unknown>
}

interface SeedMessageService {
  find(params: { query: { $limit: number }, paginate: false }): Promise<MessageSeedRow[] | FindRows<MessageSeedRow>>
  create(data: { text: string }, params: { user: { userId: string } }): Promise<unknown>
}
...
async function ensureMongoIndexes(app: Application): Promise<void> {
  const db = app.get?.('mongodbDb') as Db | undefined
  if (!db)
    return
...
export default async function seedUsers(app: Application, _ctx: SeedContext = {}) {
  ...
  const users = app.service('users') as unknown as SeedUserService
  const messages = app.service('messages') as unknown as SeedMessageService
  ...
  if (!messageRows.length) {
    await messages.create({
      text: 'Bienvenue dans l’exemple NFZ + DaisyUiKit + MongoDB + Redis.',
    }, {
      user: { userId },
    })
  }
