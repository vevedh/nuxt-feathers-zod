...
Exemple source maintenu pour **nuxt-feathers-zod 6.7.51**. Il montre une application métier complète avec Nuxt 4, Vue 3, DaisyUiKit, **Tailwind CSS 4** (choisi ici parmi l'alternative UnoCSS/Tailwind), Pinia, MongoDB, FeathersJS v5 embedded, auth locale/JWT, RBAC `admin/member`, Redis et thèmes DaisyUI.

> NFZ 6.7.51 ne possède pas d'option publique `feathers.cache`. Le cache Redis de cet exemple utilise la couche serveur Nitro/Unstorage. Le cœur NFZ reste inchangé.
...
Dans la base 6.7.51, `ModuleOptions` ne déclare pas de clé `cache` ou `redis`. Ajouter une fausse configuration `feathers.cache` serait donc trompeur. L'intégration recommandée aujourd'hui est :
