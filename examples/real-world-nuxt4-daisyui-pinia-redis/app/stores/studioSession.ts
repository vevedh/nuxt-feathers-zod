import type { LocalAuthLikeOptions } from 'nuxt-feathers-zod/auth-utils'
...
function readLocalAuthOptions(value: unknown): LocalAuthLikeOptions | undefined {
  if (!isRecord(value))
    return undefined
  const feathers = value._feathers
  if (!isRecord(feathers) || !isRecord(feathers.auth) || !isRecord(feathers.auth.local))
    return undefined

  const local = feathers.auth.local
  return {
    usernameField: typeof local.usernameField === 'string' ? local.usernameField : undefined,
    passwordField: typeof local.passwordField === 'string' ? local.passwordField : undefined,
  }
}
...
export const useStudioSessionStore = defineStore('studioSession', () => {
  ...
  async function login(credentials: LoginCredentials): Promise<void> {
    const localAuth = readLocalAuthOptions(runtimeConfig.public)
